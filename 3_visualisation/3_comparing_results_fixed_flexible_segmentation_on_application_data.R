...
for.suc.yday.fixed$for.suc <- for.suc.yday.fixed$nobs.ingest/for.suc.yday.fixed$nobs.foraging
# FLEXIBLE
...
for.suc.yday.flex$for.suc <- for.suc.yday.flex$nobs.ingest/for.suc.yday.flex$nobs.foraging
mean.for.suc.yday.flex <- calculate.mean.95CI.from.logits(for.suc.yday.flex)
...
# FIXED
plotCI(mean.for.suc.yday.fixed$yday, mean.for.suc.yday.fixed$mean, li=mean.for.suc.yday.fixed$li, ui=mean.for.suc.yday.fixed$ui, xlab="", ylab = "", ylim=c(0,0.2), xlim=c(min(mean.for.suc.yday.fixed$yday),max(mean.for.suc.yday.fixed$yday)), pch=21, sfrac=0, gap=0, pt.bg="white", col="steelblue2", xaxt='n', las=1)
# FLEXIBLE
...
mtext("Prey ingestion rate",2,3,cex=1.2)
dev.off()
### END FIGURE S5 ###

median((mean.for.suc.yday.fixed$mean/mean.for.suc.yday.flex$mean)) # ratio 1.34 (or 0.75)
median((mean.for.suc.yday.fixed$mean-mean.for.suc.yday.flex$mean)/mean.for.suc.yday.fixed$mean) # 25% lower estimated prey ingestion rates using flexible relative to fixed segmentation method

# Data preparation for figures S7 and S8 
...
prop.foraging.day.fixed$ingest.rate <- round(prop.foraging.day.fixed$nobs.ingest/prop.foraging.day.fixed$nobs.foraging,3)
...
prop.foraging.day.flex$ingest.rate <- round(prop.foraging.day.flex$nobs.ingest/prop.foraging.day.flex$nobs.foraging,3)
...
prop.foraging.day.wadden.fixed$ingest.rate <- round(prop.foraging.day.wadden.fixed$nobs.ingest/prop.foraging.day.wadden.fixed$nobs.foraging,3)
...
prop.foraging.day.wadden.flex$ingest.rate <- round(prop.foraging.day.wadden.flex$nobs.ingest/prop.foraging.day.wadden.flex$nobs.foraging,3)
...
par(mar=c(1,5,0,0), oma=c(1,0,3,1), mgp=c(3.5,1,0), padj=0.5)
plot(prop.for~yday, prop.foraging.day.flex, ylab="Perc. time foraging", pch=21, col="orange2", xaxt="n", yaxt="n", ylim=c(0,0.55), cex.lab=1.5)
mtext("All data",3,1,cex=1.3)
points(prop.for~yday, prop.foraging.day.fixed, pch=21, col="steelblue2")
...
plot(ingest.rate*60/0.8~yday, prop.foraging.day.flex, ylab="", pch=21, col="orange2", ylim=c(0,10), xaxt="n", las=1, cex.lab=1.5, cex.axis=1.3)
mtext(expression("Prey ingestion rate ( "*min^-1*" )"),2,3)
points(ingest.rate*60/0.8~yday, prop.foraging.day.fixed, pch=21, col="steelblue2")
axis(1, at=c(31+28+31+c(1,30+1,30+31+1,30+31+30+1,30+31+30+31+1,30+31+30+31+31+1,30+31+30+31+31+30+1)), labels=F)
...
# Wadden Sea data:
plot(prop.for~yday, prop.foraging.day.wadden.flex, ylab="", pch=21, col="orange2", xaxt="n", yaxt="n", ylim=c(0,1), cex.lab=1.5)
mtext("Wadden Sea data",3,1,cex=1.3)
points(prop.for~yday, prop.foraging.day.wadden.fixed, pch=21, col="steelblue2")
axis(1, at=c(31+28+31+c(1,30+1,30+31+1,30+31+30+1,30+31+30+31+1,30+31+30+31+31+1,30+31+30+31+31+30+1)), labels=F)
axis(2, at=seq(0,1,0.2), labels=paste(seq(0,100,20),"%",sep=""), line=0, las=1, cex.axis=1.3)
...
plot(ingest.rate*60/0.8~yday, prop.foraging.day.wadden.flex, ylab="", pch=21, col="orange2", ylim=c(0,12), xaxt="n", las=1, cex.lab=1.5, cex.axis=1.3)
points(ingest.rate*60/0.8~yday, prop.foraging.day.wadden.fixed, pch=21, col="steelblue2")
axis(1, at=c(31+28+31+c(1,30+1,30+31+1,30+31+30+1,30+31+30+31+1,30+31+30+31+31+1,30+31+30+31+31+30+1)), labels=F)
axis(1, at=c(31+28+31+c(1,30+1,30+31+1,30+31+30+1,30+31+30+31+1,30+31+30+31+31+1)+15), labels=c("Apr","May","June","July","Aug","Sep"), tick=F, line=-0.5, cex.axis=1.3)
dev.off()
### END Figure S7 ### 
