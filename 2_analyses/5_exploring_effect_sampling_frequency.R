...
sensitivity.sampling.freq.nsim <- array(NA, c(nsim,length(behaviour.labels),length(sampling.freqs)), dimnames = list(1:nsim, behaviour.labels, sampling.freqs))
precision.sampling.freq.nsim <-array(NA, c(nsim,length(behaviour.labels),length(sampling.freqs)), dimnames = list(1:nsim, behaviour.labels, sampling.freqs))

sensitivity.pooled.behaviours.sampling.freq.nsim <- array(NA, c(nsim,4,length(sampling.freqs)), dimnames = list(1:nsim, c("flap","soar","active.ground","rest"), sampling.freqs))
precision.pooled.behaviours.sampling.freq.nsim <-array(NA, c(nsim,4,length(sampling.freqs)), dimnames = list(1:nsim, c("flap","soar","active.ground","rest"), sampling.freqs))
